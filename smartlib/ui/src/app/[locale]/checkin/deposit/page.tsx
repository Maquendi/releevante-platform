...
export default function DepositPage() {
  ...
  const { currentItem } = useAppSelector((state) => state.bookExchange);
  const router = useRouter();
  useEffect(() => {
    if (currentItem?.exchangeCompleted) {
        ...
        <div>
          <h1 className="text-3xl font-medium">
            ...
            <span className="text-primary">
              {t("depositBookTitle2")} <span> {currentItem?.position} </span>
            </span>
          </h1>
        </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm">
                  ...
                  <span className="text-primary px-1">
                    &lt; {t("firstStepContent2")} {currentItem?.position} &gt;
                  </span>
                  {t("firstStepContent3")}
                </p>
              </div>
